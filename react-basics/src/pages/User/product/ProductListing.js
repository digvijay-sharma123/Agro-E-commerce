...
const ProductListing = () => {
  ...
  const addToCart = async (productId) => {
    await axios.post(`http://localhost:3001/cart`, { 
      ...
      .then((res) => {
        const updatedProduct = res.data.updated_product;
        const updatedProductList = productList.map(product => {
          if (product.product_id === productId) {
            return { ...product, qty: updatedProduct.qty }; // Update qty here
          } else {
            return {
              ...product
            }
        ...
        {isProductListEmpty ? <h3>No products found!</h3> : productList.map(product => (product.qty > 0 ?
          <div className="product-card" key={product.product_id}>
            ...
            <div className="product-info">
              ...
              <button
                onClick={() => product.product_id ? addToCart(product.product_id) : console.error("Product ID is undefined")}
              >
                &#8369;{product.price ? product.price.toFixed(2) : '0.00'} Add to Cart
              </button>
